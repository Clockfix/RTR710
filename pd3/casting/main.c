...
int main(){
    ...
    int32_t *ptr_int; //pointeris

    float *ptr_float;

    uint16_t val_uint16t;

    // left: int32_t; rigt: uint8_t*
    // ptr_int = arr;
    ptr_int = (int32_t*)arr;  // ieraksta adresi arr[0]

    printf("addr:%x HEX:0x%x DEC:%d\n", &ptr_float, *&*&*&*ptr_int, *ptr_int);
    

    // left: int32_t; rigt: uint8_t8
    ptr_float = (float*)arr;  // ieraksta adresi arr[0]

    printf("addr:%x HEX:0x%x DEC:%f\n", ptr_float, *ptr_float, *ptr_float);

    // left: uint16_t; rigt: uint8_t*
